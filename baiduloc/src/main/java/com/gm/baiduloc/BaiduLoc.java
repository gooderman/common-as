...
public class BaiduLoc {
	//-----------------------------------------------------
	public static String BD_MAP_KEY = "";
	//-----------------------------------------------------
	...
	public static void nativeNotifyLocation(int error, double longitude,  double latitude, String address,String describe)
	{
		s_loc_listener.onLocationResult(error,longitude,latitude,address,describe);
	}
		...
		locationListener = new BDLocationListener() {
			...
			public void onReceiveLocation(BDLocation location) {
				...
				if(null == location ||
					...
					nativeNotifyLocation(0,s_location.getLongitude(),s_location.getLatitude(),s_address_str,decribe);
	...
	public static boolean reqPermission(){
		if (Build.VERSION.SDK_INT >= 23) {
			...
			int idx = 0;
			String[] tmp = new String[ps.length];
			for (String s : ps) {
				if (m_context.checkSelfPermission(s) != PackageManager.PERMISSION_GRANTED) {
					tmp[idx] = s;
					idx++;
				}
			}
			...
			String[] nps = new String[idx];
			for (int i = 0; i < idx; i++) {
				nps[i] = tmp[i];
			}
			m_context.requestPermissions(nps, 100);
			return false;
		}
		else{
			return true;
